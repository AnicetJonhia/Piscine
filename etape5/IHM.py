...
def envoyer_mail():
    ...
    selected_items = treeview.selection()  # Obtenez les lignes sélectionnées dans la liste
    receivers = []

    for item in selected_items:
        receivers.append(treeview.item(item, "values")[0])

    # Envoie l'e-mail aux destinataires sélectionnés
    for receiver in receivers:
        sendMail.sendMail(email_address, password, receiver)
...
email_address_label = ttk.Label(frame, text="Adresse e-mail :")
email_address_label.grid(row=1, column=0, padx=10, pady=5, sticky="w")

email_address_entry = ttk.Entry(frame)
email_address_entry.grid(row=1, column=1, padx=10, pady=5, sticky="w")

password_label = ttk.Label(frame, text="Mot de passe :")
password_label.grid(row=2, column=0, padx=10, pady=5, sticky="w")

password_entry = ttk.Entry(frame, show="*")  # Pour masquer le mot de passe
